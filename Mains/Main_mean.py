'''
Mean predicted score as an individual predicted score.
'''
...
def main():
    ...
    for fold, (train_index, test_index) in enumerate(skf.split(np.zeros(len(labels)), labels)):
        ...
        for i, file in enumerate(test_files):
            ...
            individual_predictions[individual_id].append(prediction)
            individual_labels[individual_id] = individual_label

        average_individual_predictions = {}
        for individual_id, predictions in individual_predictions.items():
            average_prediction = np.mean(predictions)
            average_individual_predictions[individual_id] = average_prediction

        true_labels = [individual_labels[individual_id] for individual_id in average_individual_predictions]
        average_predictions = [average_individual_predictions[individual_id] for individual_id in
                               average_individual_predictions]

        # Calculate AUC for the current fold
        model_auc = roc_auc_score(true_labels, average_predictions)
        ...
        tn, fp, fn, tp = confusion_matrix(true_labels, [1 if p > 0.5 else 0 for p in average_predictions]).ravel()
